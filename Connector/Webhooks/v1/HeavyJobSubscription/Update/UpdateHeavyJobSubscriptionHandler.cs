...
public class UpdateHeavyJobSubscriptionHandler : IActionHandler<UpdateHeavyJobSubscriptionAction>
{
    private readonly ILogger<UpdateHeavyJobSubscriptionHandler> _logger;

    public UpdateHeavyJobSubscriptionHandler(
        ILogger<UpdateHeavyJobSubscriptionHandler> logger)
    {
        _logger = logger;
    }
    ...
    {
        var input = JsonSerializer.Deserialize<UpdateHeavyJobSubscriptionActionInput>(actionInstance.InputJson);
        try
        {
            // Given the input for the action, make a call to your API/system
            var response = new ApiResponse<UpdateHeavyJobSubscriptionActionOutput>();
            // response = await _apiClient.PostHeavyJobSubscriptionDataObject(input, cancellationToken)
            ...
            // var resource = await _apiClient.GetHeavyJobSubscriptionDataObject(response.Data.id, cancellationToken);

            // var resource = new UpdateHeavyJobSubscriptionActionOutput
            // {
            //      TODO : map
            // };
            ...
            {
                new CacheSyncCollection() { DataObjectType = typeof(HeavyJobSubscriptionDataObject), CacheChanges = operations.ToArray() }
            };
        ...
        catch (HttpRequestException exception)
        {
            ...
            var errorSource = new List<string> { "UpdateHeavyJobSubscriptionHandler" };
