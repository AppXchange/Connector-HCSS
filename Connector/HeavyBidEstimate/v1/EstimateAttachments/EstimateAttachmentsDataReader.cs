...
public class EstimateAttachmentsDataReader : TypedAsyncDataReaderBase<EstimateAttachmentsDataObject>
{
    ...
    public override async IAsyncEnumerable<EstimateAttachmentsDataObject> GetTypedDataAsync(
        ...
        [EnumeratorCancellation] CancellationToken cancellationToken)
    {
        ...
        while (true)
        {
            ApiResponse<HeavyBidResponse<EstimateAttachmentsDataObject>> response;
            try
            {
                response = await _apiClient.GetEstimateAttachments(
            ...
            {
                _logger.LogError(exception, "Exception while retrieving estimate attachments");
                throw;
            }

            if (!response.IsSuccessful || response.Data?.Data == null)
            {
                _logger.LogError("Failed to retrieve estimate attachments. Status code: {StatusCode}", response.StatusCode);
                throw new Exception($"Failed to retrieve estimate attachments. API StatusCode: {response.StatusCode}");
            }

            foreach (var attachment in response.Data.Data)
            {
