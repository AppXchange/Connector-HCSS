...
public class CreateSubletVendorCostEntriesHandler : IActionHandler<CreateSubletVendorCostEntriesAction>
{
    private readonly ILogger<CreateSubletVendorCostEntriesHandler> _logger;

    public CreateSubletVendorCostEntriesHandler(
        ILogger<CreateSubletVendorCostEntriesHandler> logger)
    {
        _logger = logger;
    }
    ...
    {
        var input = JsonSerializer.Deserialize<CreateSubletVendorCostEntriesActionInput>(actionInstance.InputJson);
        try
        {
            // Given the input for the action, make a call to your API/system
            var response = new ApiResponse<CreateSubletVendorCostEntriesActionOutput>();
            // response = await _apiClient.PostSubletVendorCostEntriesDataObject(input, cancellationToken)
            ...
            // var resource = await _apiClient.GetSubletVendorCostEntriesDataObject(response.Data.id, cancellationToken);

            // var resource = new CreateSubletVendorCostEntriesActionOutput
            // {
            //      TODO : map
            // };
            ...
            {
                new CacheSyncCollection() { DataObjectType = typeof(SubletVendorCostEntriesDataObject), CacheChanges = operations.ToArray() }
            };
        ...
        catch (HttpRequestException exception)
        {
            ...
            var errorSource = new List<string> { "CreateSubletVendorCostEntriesHandler" };
