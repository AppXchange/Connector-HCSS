...
public class CreateWorkOrderHandler : IActionHandler<CreateWorkOrderAction>
{
    ...
    {
        var input = JsonSerializer.Deserialize<CreateWorkOrderActionInput>(actionInstance.InputJson);
        try
        {
            // Given the input for the action, make a call to your API/system
            var response = new ApiResponse<CreateWorkOrderActionOutput>();
            // response = await _apiClient.PostWorkOrderDataObject(input, cancellationToken)
            ...
            // var resource = await _apiClient.GetWorkOrderDataObject(response.Data.id, cancellationToken);

            // var resource = new CreateWorkOrderActionOutput
            // {
            //      TODO : map
            // };
            ...
            {
                new CacheSyncCollection() { DataObjectType = typeof(WorkOrderDataObject), CacheChanges = operations.ToArray() }
            };
        ...
        catch (HttpRequestException exception)
        {
            ...
            var errorSource = new List<string> { "CreateWorkOrderHandler" };
