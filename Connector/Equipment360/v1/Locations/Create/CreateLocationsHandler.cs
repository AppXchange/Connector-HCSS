...
public class CreateLocationsHandler : IActionHandler<CreateLocationsAction>
{
    ...
    {
        var input = JsonSerializer.Deserialize<CreateLocationsActionInput>(actionInstance.InputJson);
        try
        {
            // Given the input for the action, make a call to your API/system
            var response = new ApiResponse<CreateLocationsActionOutput>();
            // response = await _apiClient.PostLocationsDataObject(input, cancellationToken)
            ...
            // var resource = await _apiClient.GetLocationsDataObject(response.Data.id, cancellationToken);

            // var resource = new CreateLocationsActionOutput
            // {
            //      TODO : map
            // };
            ...
            {
                new CacheSyncCollection() { DataObjectType = typeof(LocationsDataObject), CacheChanges = operations.ToArray() }
            };
        ...
        catch (HttpRequestException exception)
        {
            ...
            var errorSource = new List<string> { "CreateLocationsHandler" };
