...
public class CreateEquipmentHandler : IActionHandler<CreateEquipmentAction>
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        try
        {
            var response = await _apiClient.CreateEquipment(input, cancellationToken);

            if (!response.IsSuccessful || response.Data == null)
            {
                _logger.LogError("Failed to create equipment. Status code: {StatusCode}", response.StatusCode);
                throw new Exception($"Failed to create equipment. API StatusCode: {response.StatusCode}");
            }
        ...
        {
            _logger.LogError(exception, "Exception while creating equipment");
            
            var errorSource = new List<string> { nameof(CreateEquipmentHandler) };
