...
namespace Connector.Safety.v1.UserAccessGroups.Create;

public class CreateUserAccessGroupsHandler : IActionHandler<CreateUserAccessGroupsAction>
{
    private readonly ILogger<CreateUserAccessGroupsHandler> _logger;

    public CreateUserAccessGroupsHandler(
        ILogger<CreateUserAccessGroupsHandler> logger)
    {
        _logger = logger;
    }
    ...
    {
        var input = JsonSerializer.Deserialize<CreateUserAccessGroupsActionInput>(actionInstance.InputJson);
        try
        {
            // Given the input for the action, make a call to your API/system
            var response = new ApiResponse<CreateUserAccessGroupsActionOutput>();
            // response = await _apiClient.PostUserAccessGroupsDataObject(input, cancellationToken)
            ...
            // var resource = await _apiClient.GetUserAccessGroupsDataObject(response.Data.id, cancellationToken);

            // var resource = new CreateUserAccessGroupsActionOutput
            // {
            //      TODO : map
            // };
        ...
        catch (HttpRequestException exception)
        {
            ...
            var errorSource = new List<string> { "CreateUserAccessGroupsHandler" };
