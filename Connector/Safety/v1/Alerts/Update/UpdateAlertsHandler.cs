...
namespace Connector.Safety.v1.Alerts.Update;

public class UpdateAlertsHandler : IActionHandler<UpdateAlertsAction>
{
    private readonly ILogger<UpdateAlertsHandler> _logger;

    public UpdateAlertsHandler(
        ILogger<UpdateAlertsHandler> logger)
    {
        _logger = logger;
    }
    ...
    {
        var input = JsonSerializer.Deserialize<UpdateAlertsActionInput>(actionInstance.InputJson);
        try
        {
            // Given the input for the action, make a call to your API/system
            var response = new ApiResponse<UpdateAlertsActionOutput>();
            // response = await _apiClient.PostAlertsDataObject(input, cancellationToken)
            ...
            // var resource = await _apiClient.GetAlertsDataObject(response.Data.id, cancellationToken);

            // var resource = new UpdateAlertsActionOutput
            // {
            //      TODO : map
            // };
            ...
            {
                new CacheSyncCollection() { DataObjectType = typeof(AlertsDataObject), CacheChanges = operations.ToArray() }
            };
        ...
        catch (HttpRequestException exception)
        {
            ...
            var errorSource = new List<string> { "UpdateAlertsHandler" };
