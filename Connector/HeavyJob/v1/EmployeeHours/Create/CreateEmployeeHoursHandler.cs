...
public class CreateEmployeeHoursHandler : IActionHandler<CreateEmployeeHoursAction>
{
    private readonly ILogger<CreateEmployeeHoursHandler> _logger;

    public CreateEmployeeHoursHandler(
        ILogger<CreateEmployeeHoursHandler> logger)
    {
        _logger = logger;
    }
    ...
    {
        var input = JsonSerializer.Deserialize<CreateEmployeeHoursActionInput>(actionInstance.InputJson);
        try
        {
            // Given the input for the action, make a call to your API/system
            var response = new ApiResponse<CreateEmployeeHoursActionOutput>();
            // response = await _apiClient.PostEmployeeHoursDataObject(input, cancellationToken)
            ...
            // var resource = await _apiClient.GetEmployeeHoursDataObject(response.Data.id, cancellationToken);

            // var resource = new CreateEmployeeHoursActionOutput
            // {
            //      TODO : map
            // };
            ...
            {
                new CacheSyncCollection() { DataObjectType = typeof(EmployeeHoursDataObject), CacheChanges = operations.ToArray() }
            };
        ...
        catch (HttpRequestException exception)
        {
            ...
            var errorSource = new List<string> { "CreateEmployeeHoursHandler" };
