...
namespace Connector.HeavyJob.v1.JobCostsToDate.Create;

public class CreateJobCostsToDateHandler : IActionHandler<CreateJobCostsToDateAction>
{
    private readonly ILogger<CreateJobCostsToDateHandler> _logger;

    public CreateJobCostsToDateHandler(
        ILogger<CreateJobCostsToDateHandler> logger)
    {
        _logger = logger;
    }
    ...
    {
        var input = JsonSerializer.Deserialize<CreateJobCostsToDateActionInput>(actionInstance.InputJson);
        try
        {
            // Given the input for the action, make a call to your API/system
            var response = new ApiResponse<CreateJobCostsToDateActionOutput>();
            // response = await _apiClient.PostJobCostsToDateDataObject(input, cancellationToken)
            ...
            // var resource = await _apiClient.GetJobCostsToDateDataObject(response.Data.id, cancellationToken);

            // var resource = new CreateJobCostsToDateActionOutput
            // {
            //      TODO : map
            // };
            ...
            {
                new CacheSyncCollection() { DataObjectType = typeof(JobCostsToDateDataObject), CacheChanges = operations.ToArray() }
            };
        ...
        catch (HttpRequestException exception)
        {
            ...
            var errorSource = new List<string> { "CreateJobCostsToDateHandler" };
