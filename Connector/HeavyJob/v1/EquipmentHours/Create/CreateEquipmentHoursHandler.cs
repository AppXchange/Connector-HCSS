...
namespace Connector.HeavyJob.v1.EquipmentHours.Create;

public class CreateEquipmentHoursHandler : IActionHandler<CreateEquipmentHoursAction>
{
    private readonly ILogger<CreateEquipmentHoursHandler> _logger;

    public CreateEquipmentHoursHandler(
        ILogger<CreateEquipmentHoursHandler> logger)
    {
        _logger = logger;
    }
    ...
    {
        var input = JsonSerializer.Deserialize<CreateEquipmentHoursActionInput>(actionInstance.InputJson);
        try
        {
            // Given the input for the action, make a call to your API/system
            var response = new ApiResponse<CreateEquipmentHoursActionOutput>();
            // response = await _apiClient.PostEquipmentHoursDataObject(input, cancellationToken)
            ...
            // var resource = await _apiClient.GetEquipmentHoursDataObject(response.Data.id, cancellationToken);

            // var resource = new CreateEquipmentHoursActionOutput
            // {
            //      TODO : map
            // };
            ...
            {
                new CacheSyncCollection() { DataObjectType = typeof(EquipmentHoursDataObject), CacheChanges = operations.ToArray() }
            };
        ...
        catch (HttpRequestException exception)
        {
            ...
            var errorSource = new List<string> { "CreateEquipmentHoursHandler" };
