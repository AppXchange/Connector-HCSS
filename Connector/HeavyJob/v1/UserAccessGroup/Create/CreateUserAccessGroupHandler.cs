...
namespace Connector.HeavyJob.v1.UserAccessGroup.Create;

public class CreateUserAccessGroupHandler : IActionHandler<CreateUserAccessGroupAction>
{
    private readonly ILogger<CreateUserAccessGroupHandler> _logger;

    public CreateUserAccessGroupHandler(
        ILogger<CreateUserAccessGroupHandler> logger)
    {
        _logger = logger;
    }
    ...
    {
        var input = JsonSerializer.Deserialize<CreateUserAccessGroupActionInput>(actionInstance.InputJson);
        try
        {
            // Given the input for the action, make a call to your API/system
            var response = new ApiResponse<CreateUserAccessGroupActionOutput>();
            // response = await _apiClient.PostUserAccessGroupDataObject(input, cancellationToken)
            ...
            // var resource = await _apiClient.GetUserAccessGroupDataObject(response.Data.id, cancellationToken);

            // var resource = new CreateUserAccessGroupActionOutput
            // {
            //      TODO : map
            // };
            ...
            {
                new CacheSyncCollection() { DataObjectType = typeof(UserAccessGroupDataObject), CacheChanges = operations.ToArray() }
            };
        ...
        catch (HttpRequestException exception)
        {
            ...
            var errorSource = new List<string> { "CreateUserAccessGroupHandler" };
