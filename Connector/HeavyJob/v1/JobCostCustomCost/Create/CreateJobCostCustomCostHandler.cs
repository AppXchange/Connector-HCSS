...
namespace Connector.HeavyJob.v1.JobCostCustomCost.Create;

public class CreateJobCostCustomCostHandler : IActionHandler<CreateJobCostCustomCostAction>
{
    private readonly ILogger<CreateJobCostCustomCostHandler> _logger;

    public CreateJobCostCustomCostHandler(
        ILogger<CreateJobCostCustomCostHandler> logger)
    {
        _logger = logger;
    }
    ...
    {
        var input = JsonSerializer.Deserialize<CreateJobCostCustomCostActionInput>(actionInstance.InputJson);
        try
        {
            // Given the input for the action, make a call to your API/system
            var response = new ApiResponse<CreateJobCostCustomCostActionOutput>();
            // response = await _apiClient.PostJobCostCustomCostDataObject(input, cancellationToken)
            ...
            // var resource = await _apiClient.GetJobCostCustomCostDataObject(response.Data.id, cancellationToken);

            // var resource = new CreateJobCostCustomCostActionOutput
            // {
            //      TODO : map
            // };
            ...
            {
                new CacheSyncCollection() { DataObjectType = typeof(JobCostCustomCostDataObject), CacheChanges = operations.ToArray() }
            };
        ...
        catch (HttpRequestException exception)
        {
            ...
            var errorSource = new List<string> { "CreateJobCostCustomCostHandler" };
