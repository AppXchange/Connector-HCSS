...
public class DeleteJobCustomCostTypeItemHandler : IActionHandler<DeleteJobCustomCostTypeItemAction>
{
    private readonly ILogger<DeleteJobCustomCostTypeItemHandler> _logger;

    public DeleteJobCustomCostTypeItemHandler(
        ILogger<DeleteJobCustomCostTypeItemHandler> logger)
    {
        _logger = logger;
    }
    ...
    {
        var input = JsonSerializer.Deserialize<DeleteJobCustomCostTypeItemActionInput>(actionInstance.InputJson);
        try
        {
            // Given the input for the action, make a call to your API/system
            var response = new ApiResponse<DeleteJobCustomCostTypeItemActionOutput>();
            // response = await _apiClient.PostJobCustomCostTypeItemDataObject(input, cancellationToken)
            ...
            // var resource = await _apiClient.GetJobCustomCostTypeItemDataObject(response.Data.id, cancellationToken);

            // var resource = new DeleteJobCustomCostTypeItemActionOutput
            // {
            //      TODO : map
            // };
            ...
            {
                new CacheSyncCollection() { DataObjectType = typeof(JobCustomCostTypeItemDataObject), CacheChanges = operations.ToArray() }
            };
        ...
        catch (HttpRequestException exception)
        {
            ...
            var errorSource = new List<string> { "DeleteJobCustomCostTypeItemHandler" };
