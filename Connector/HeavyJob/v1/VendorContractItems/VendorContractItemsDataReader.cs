...
public class VendorContractItemsDataReader : TypedAsyncDataReaderBase<VendorContractItemsDataObject>
{
    ...
    public override async IAsyncEnumerable<VendorContractItemsDataObject> GetTypedDataAsync(
        ...
        [EnumeratorCancellation] CancellationToken cancellationToken)
    {
        ...
        while (true)
        {
            var response = await _apiClient.GetVendorContractItems(
                jobId: jobId,
            ...
            {
                _logger.LogError("Failed to retrieve vendor contract items. Status code: {StatusCode}", response.StatusCode);
                throw new Exception($"Failed to retrieve vendor contract items. API StatusCode: {response.StatusCode}");
            }
