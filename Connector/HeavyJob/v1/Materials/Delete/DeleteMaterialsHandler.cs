...
namespace Connector.HeavyJob.v1.Materials.Delete;

public class DeleteMaterialsHandler : IActionHandler<DeleteMaterialsAction>
{
    private readonly ILogger<DeleteMaterialsHandler> _logger;

    public DeleteMaterialsHandler(
        ILogger<DeleteMaterialsHandler> logger)
    {
        _logger = logger;
    }
    ...
    {
        var input = JsonSerializer.Deserialize<DeleteMaterialsActionInput>(actionInstance.InputJson);
        try
        {
            // Given the input for the action, make a call to your API/system
            var response = new ApiResponse<DeleteMaterialsActionOutput>();
            // response = await _apiClient.PostMaterialsDataObject(input, cancellationToken)
            ...
            // var resource = await _apiClient.GetMaterialsDataObject(response.Data.id, cancellationToken);

            // var resource = new DeleteMaterialsActionOutput
            // {
            //      TODO : map
            // };
            ...
            {
                new CacheSyncCollection() { DataObjectType = typeof(MaterialsDataObject), CacheChanges = operations.ToArray() }
            };
        ...
        catch (HttpRequestException exception)
        {
            ...
            var errorSource = new List<string> { "DeleteMaterialsHandler" };
