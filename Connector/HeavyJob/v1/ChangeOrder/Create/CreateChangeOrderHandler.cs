...
public class CreateChangeOrderHandler : IActionHandler<CreateChangeOrderAction>
{
    private readonly ILogger<CreateChangeOrderHandler> _logger;

    public CreateChangeOrderHandler(
        ILogger<CreateChangeOrderHandler> logger)
    {
        _logger = logger;
    }
    ...
    {
        var input = JsonSerializer.Deserialize<CreateChangeOrderActionInput>(actionInstance.InputJson);
        try
        {
            // Given the input for the action, make a call to your API/system
            var response = new ApiResponse<CreateChangeOrderActionOutput>();
            // response = await _apiClient.PostChangeOrderDataObject(input, cancellationToken)
            ...
            // var resource = await _apiClient.GetChangeOrderDataObject(response.Data.id, cancellationToken);

            // var resource = new CreateChangeOrderActionOutput
            // {
            //      TODO : map
            // };
            ...
            {
                new CacheSyncCollection() { DataObjectType = typeof(ChangeOrderDataObject), CacheChanges = operations.ToArray() }
            };
        ...
        catch (HttpRequestException exception)
        {
            ...
            var errorSource = new List<string> { "CreateChangeOrderHandler" };
