...
public class CreateDiariesHandler : IActionHandler<CreateDiariesAction>
{
    private readonly ILogger<CreateDiariesHandler> _logger;

    public CreateDiariesHandler(
        ILogger<CreateDiariesHandler> logger)
    {
        _logger = logger;
    }
    ...
    {
        var input = JsonSerializer.Deserialize<CreateDiariesActionInput>(actionInstance.InputJson);
        try
        {
            // Given the input for the action, make a call to your API/system
            var response = new ApiResponse<CreateDiariesActionOutput>();
            // response = await _apiClient.PostDiariesDataObject(input, cancellationToken)
            ...
            // var resource = await _apiClient.GetDiariesDataObject(response.Data.id, cancellationToken);

            // var resource = new CreateDiariesActionOutput
            // {
            //      TODO : map
            // };
            ...
            {
                new CacheSyncCollection() { DataObjectType = typeof(DiariesDataObject), CacheChanges = operations.ToArray() }
            };
        ...
        catch (HttpRequestException exception)
        {
            ...
            var errorSource = new List<string> { "CreateDiariesHandler" };
