...
public class UpdateMaterialPurchaseOrderDetailsHandler : IActionHandler<UpdateMaterialPurchaseOrderDetailsAction>
{
    private readonly ILogger<UpdateMaterialPurchaseOrderDetailsHandler> _logger;

    public UpdateMaterialPurchaseOrderDetailsHandler(
        ILogger<UpdateMaterialPurchaseOrderDetailsHandler> logger)
    {
        _logger = logger;
    }
    ...
    {
        var input = JsonSerializer.Deserialize<UpdateMaterialPurchaseOrderDetailsActionInput>(actionInstance.InputJson);
        try
        {
            // Given the input for the action, make a call to your API/system
            var response = new ApiResponse<UpdateMaterialPurchaseOrderDetailsActionOutput>();
            // response = await _apiClient.PostMaterialPurchaseOrderDetailsDataObject(input, cancellationToken)
            ...
            // var resource = await _apiClient.GetMaterialPurchaseOrderDetailsDataObject(response.Data.id, cancellationToken);

            // var resource = new UpdateMaterialPurchaseOrderDetailsActionOutput
            // {
            //      TODO : map
            // };
            ...
            {
                new CacheSyncCollection() { DataObjectType = typeof(MaterialPurchaseOrderDetailsDataObject), CacheChanges = operations.ToArray() }
            };
        ...
        catch (HttpRequestException exception)
        {
            ...
            var errorSource = new List<string> { "UpdateMaterialPurchaseOrderDetailsHandler" };
