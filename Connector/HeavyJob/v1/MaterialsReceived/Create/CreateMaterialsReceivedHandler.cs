...
namespace Connector.HeavyJob.v1.MaterialsReceived.Create;

public class CreateMaterialsReceivedHandler : IActionHandler<CreateMaterialsReceivedAction>
{
    private readonly ILogger<CreateMaterialsReceivedHandler> _logger;

    public CreateMaterialsReceivedHandler(
        ILogger<CreateMaterialsReceivedHandler> logger)
    {
        _logger = logger;
    }
    ...
    {
        var input = JsonSerializer.Deserialize<CreateMaterialsReceivedActionInput>(actionInstance.InputJson);
        try
        {
            // Given the input for the action, make a call to your API/system
            var response = new ApiResponse<CreateMaterialsReceivedActionOutput>();
            // response = await _apiClient.PostMaterialsReceivedDataObject(input, cancellationToken)
            ...
            // var resource = await _apiClient.GetMaterialsReceivedDataObject(response.Data.id, cancellationToken);

            // var resource = new CreateMaterialsReceivedActionOutput
            // {
            //      TODO : map
            // };
            ...
            {
                new CacheSyncCollection() { DataObjectType = typeof(MaterialsReceivedDataObject), CacheChanges = operations.ToArray() }
            };
        ...
        catch (HttpRequestException exception)
        {
            ...
            var errorSource = new List<string> { "CreateMaterialsReceivedHandler" };
