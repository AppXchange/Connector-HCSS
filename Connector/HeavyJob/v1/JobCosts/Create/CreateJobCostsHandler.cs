...
public class CreateJobCostsHandler : IActionHandler<CreateJobCostsAction>
{
    private readonly ILogger<CreateJobCostsHandler> _logger;

    public CreateJobCostsHandler(
        ILogger<CreateJobCostsHandler> logger)
    {
        _logger = logger;
    }
    ...
    {
        var input = JsonSerializer.Deserialize<CreateJobCostsActionInput>(actionInstance.InputJson);
        try
        {
            // Given the input for the action, make a call to your API/system
            var response = new ApiResponse<CreateJobCostsActionOutput>();
            // response = await _apiClient.PostJobCostsDataObject(input, cancellationToken)
            ...
            // var resource = await _apiClient.GetJobCostsDataObject(response.Data.id, cancellationToken);

            // var resource = new CreateJobCostsActionOutput
            // {
            //      TODO : map
            // };
            ...
            {
                new CacheSyncCollection() { DataObjectType = typeof(JobCostsDataObject), CacheChanges = operations.ToArray() }
            };
        ...
        catch (HttpRequestException exception)
        {
            ...
            var errorSource = new List<string> { "CreateJobCostsHandler" };
