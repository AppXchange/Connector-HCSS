...
namespace Connector.HeavyJob.v1.Employee.Delete;

public class DeleteEmployeeHandler : IActionHandler<DeleteEmployeeAction>
{
    ...
    {
        var input = JsonSerializer.Deserialize<DeleteEmployeeActionInput>(actionInstance.InputJson);
        try
        {
            // Given the input for the action, make a call to your API/system
            var response = new ApiResponse<DeleteEmployeeActionOutput>();
            // response = await _apiClient.PostEmployeeDataObject(input, cancellationToken)
            ...
            // var resource = await _apiClient.GetEmployeeDataObject(response.Data.id, cancellationToken);

            // var resource = new DeleteEmployeeActionOutput
            // {
            //      TODO : map
            // };
        ...
        catch (HttpRequestException exception)
        {
            ...
            var errorSource = new List<string> { "DeleteEmployeeHandler" };
