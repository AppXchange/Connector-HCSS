...
public class CreatePayClassHandler : IActionHandler<CreatePayClassAction>
{
    private readonly ILogger<CreatePayClassHandler> _logger;

    public CreatePayClassHandler(
        ILogger<CreatePayClassHandler> logger)
    {
        _logger = logger;
    }
    ...
    {
        var input = JsonSerializer.Deserialize<CreatePayClassActionInput>(actionInstance.InputJson);
        try
        {
            // Given the input for the action, make a call to your API/system
            var response = new ApiResponse<CreatePayClassActionOutput>();
            // response = await _apiClient.PostPayClassDataObject(input, cancellationToken)
            ...
            // var resource = await _apiClient.GetPayClassDataObject(response.Data.id, cancellationToken);

            // var resource = new CreatePayClassActionOutput
            // {
            //      TODO : map
            // };
            ...
            {
                new CacheSyncCollection() { DataObjectType = typeof(PayClassDataObject), CacheChanges = operations.ToArray() }
            };
        ...
        catch (HttpRequestException exception)
        {
            ...
            var errorSource = new List<string> { "CreatePayClassHandler" };
