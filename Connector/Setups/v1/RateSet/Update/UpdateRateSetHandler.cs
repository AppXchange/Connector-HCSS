...
namespace Connector.Setups.v1.RateSet.Update;

public class UpdateRateSetHandler : IActionHandler<UpdateRateSetAction>
{
    private readonly ILogger<UpdateRateSetHandler> _logger;

    public UpdateRateSetHandler(
        ILogger<UpdateRateSetHandler> logger)
    {
        _logger = logger;
    }
    ...
    {
        var input = JsonSerializer.Deserialize<UpdateRateSetActionInput>(actionInstance.InputJson);
        try
        {
            // Given the input for the action, make a call to your API/system
            var response = new ApiResponse<UpdateRateSetActionOutput>();
            // response = await _apiClient.PostRateSetDataObject(input, cancellationToken)
            ...
            // var resource = await _apiClient.GetRateSetDataObject(response.Data.id, cancellationToken);

            // var resource = new UpdateRateSetActionOutput
            // {
            //      TODO : map
            // };
        ...
        catch (HttpRequestException exception)
        {
            ...
            var errorSource = new List<string> { "UpdateRateSetHandler" };
