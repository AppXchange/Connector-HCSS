...
public class CreateRateSetCostAdjustmentHandler : IActionHandler<CreateRateSetCostAdjustmentAction>
{
    private readonly ILogger<CreateRateSetCostAdjustmentHandler> _logger;

    public CreateRateSetCostAdjustmentHandler(
        ILogger<CreateRateSetCostAdjustmentHandler> logger)
    {
        _logger = logger;
    }
    ...
    {
        var input = JsonSerializer.Deserialize<CreateRateSetCostAdjustmentActionInput>(actionInstance.InputJson);
        try
        {
            // Given the input for the action, make a call to your API/system
            var response = new ApiResponse<CreateRateSetCostAdjustmentActionOutput>();
            // response = await _apiClient.PostRateSetCostAdjustmentDataObject(input, cancellationToken)
            ...
            // var resource = await _apiClient.GetRateSetCostAdjustmentDataObject(response.Data.id, cancellationToken);

            // var resource = new CreateRateSetCostAdjustmentActionOutput
            // {
            //      TODO : map
            // };
        ...
        catch (HttpRequestException exception)
        {
            ...
            var errorSource = new List<string> { "CreateRateSetCostAdjustmentHandler" };
