...
public class UpdateCostCodeHandler : IActionHandler<UpdateCostCodeAction>
{
    ...
    public async Task<ActionHandlerOutcome> HandleQueuedActionAsync(ActionInstance actionInstance, CancellationToken cancellationToken)
    {
        ...
        try
        {
            var response = await _apiClient.UpdateSetupsCostCode(input.Id, input, cancellationToken);
            ...
            if (!response.IsSuccessful || response.Data == null)
            {
                return ActionHandlerOutcome.Failed(new StandardActionFailure
                {
                    ...
                    Errors = new[]
                    {
                        new Error
                        {
                            Source = new[] { nameof(UpdateCostCodeHandler) },
                            Text = $"Failed to update cost code. Status code: {response.StatusCode}"
                        }
                    }
                });
            }
        ...
        {
            _logger.LogError(ex, "Error updating cost code");
            return ActionHandlerOutcome.Failed(new StandardActionFailure
            {
                Code = "500",
                Errors = new[]
